  ...
  await page.route(/\/api\/user\/(\d+)$/, async (route) => {
    ...
    if (method === "PUT") {
      ...
      if (loggedInUser) {
        loggedInUser = {
          ...loggedInUser,
          name: updatedUserData.name || loggedInUser.name,
          email: updatedUserData.email || loggedInUser.email,
          // Don't update password in the mock user object for security
        };

        // Also update in validUsers if it exists there
        if (loggedInUser.email && validUsers[loggedInUser.email]) {
          validUsers[loggedInUser.email] = { ...loggedInUser };
        }

        // If email changed, update the validUsers key
        if (
          updatedUserData.email &&
          updatedUserData.email !== loggedInUser.email
        ) {
          const oldEmail = loggedInUser.email;
          loggedInUser.email = updatedUserData.email;
          validUsers[updatedUserData.email] = { ...loggedInUser };
